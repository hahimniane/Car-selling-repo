...
class ParkingRecordsScreen extends StatefulWidget {
  const ParkingRecordsScreen({super.key});

  @override
  State<ParkingRecordsScreen> createState() => _ParkingRecordsScreenState();
}

class _ParkingRecordsScreenState extends State<ParkingRecordsScreen> {
  ...
  List<DocumentSnapshot> _filterRecords(List<DocumentSnapshot> allRecords) {
    ...
    if (query.isEmpty) {
      ...
      return allRecords.where((recordDoc) {
        ...
        final receiptNumber = record['receiptNumber']?.toString().toLowerCase() ?? '';
  ...
  void _showEditDialog(BuildContext context, DocumentSnapshot recordDoc) {
    ...
    final phoneController = TextEditingController(text: record['phoneNumber'] ?? '');
    final vinController = TextEditingController(text: record['vinNumber'] ?? '');
    final receiptController = TextEditingController(text: record['receiptNumber'] ?? '');
    
    // Initialize dropdown values
    String selectedStatus = record['status'] ?? 'Active';
    String? selectedMake = record['make'];
    String? selectedModel = record['model'];
    String? selectedYear = record['year']?.toString();
    DateTime? selectedParkingDate = (record['parkingDateTime'] as Timestamp?)?.toDate();
    ...
    final statusOptions = ['Active', 'Inactive', 'Completed', 'Cancelled'];
          ...
          builder: (context, setState) {
            return AlertDialog(
                      ...
                      GestureDetector(
                        onTap: () async {
                          final picked = await showDatePicker(
                            context: context,
                            initialDate: selectedParkingDate ?? DateTime.now(),
                          ...
                          if (picked != null) {
                            final timePicked = await showTimePicker(
                              context: context,
                              initialTime: TimeOfDay.fromDateTime(selectedParkingDate ?? DateTime.now()),
                            );
                            if (timePicked != null) {
                              setState(() {
                                selectedParkingDate = DateTime(
                                  picked.year,
                                  picked.month,
                                  picked.day,
                                  timePicked.hour,
                        ...
                        child: AbsorbPointer(
                          child: TextField(
                            decoration: InputDecoration(
                              labelText: 'Parking Date & Time',
                              border: const OutlineInputBorder(),
                              contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
                              suffixIcon: const Icon(Icons.calendar_today),
                            ),
                            controller: TextEditingController(
                              text: selectedParkingDate != null
                                  ? DateFormat('MMM dd, yyyy - HH:mm').format(selectedParkingDate!)
                                  : '',
                            ),
                          ),
                        ),
              ...
              actions: [
                ...
                ElevatedButton(
                  onPressed: () async {
                    await _updateParkingRecord(context, recordDoc, {
                      ...
                      'parkingDateTime': selectedParkingDate != null ? Timestamp.fromDate(selectedParkingDate!) : null,
                    });
                  },
                  style: ElevatedButton.styleFrom(
  ...
  Future<void> _updateParkingRecord(BuildContext context, DocumentSnapshot recordDoc, Map<String, dynamic> updatedData) async {
    try {
...
class _ParkingRecordCard extends StatelessWidget {
  final Map<String, dynamic> record;
  final VoidCallback onEdit;
  final VoidCallback onDelete;

  const _ParkingRecordCard({
  ...
  @override
  Widget build(BuildContext context) {
    final parkingDateTime = record['parkingDateTime'] as Timestamp?;
          ...
          children: [
            ...
            Row(
              children: [
                Expanded(
                  child: Text(
                    record['receiptNumber'] ?? 'No Receipt Number',
                    style: const TextStyle(
                      fontSize: 16,
                      fontWeight: FontWeight.bold,
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: record['status'] == 'Active' ? Colors.green : Colors.orange,
            ...
            _buildInfoRow(Icons.phone, 'Phone', record['phoneNumber'] ?? 'N/A'),
            ...
            _buildInfoRow(Icons.confirmation_number, 'VIN', record['vinNumber'] ?? 'N/A'),
            
            const SizedBox(height: 8),
            
            // Dates
            if (parkingDateTime != null)
              _buildInfoRow(Icons.schedule, 'Parking Date', 
                  DateFormat('MMM dd, yyyy - HH:mm').format(parkingDateTime.toDate())),
            if (createdAt != null)
